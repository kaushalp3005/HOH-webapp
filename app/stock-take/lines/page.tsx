...
export default function StockTakeLinesPage() {
  ...
  // Fetch open and close stock data separately
  useEffect(() => {
    const fetchStockData = async () => {
      ...
      try {
        ...
        // Fetch open stock and close stock in parallel
        const [openStockResponse, closeStockResponse] = await Promise.all([
          fetch(
            `${BACKEND_URL}/api/stock-takes/open-stock?store_name=${encodedStoreName}&open_date=${stockDate}`,
            {
              method: 'GET',
              headers: {
          ...
          fetch(
            `${BACKEND_URL}/api/stock-takes/close-stock?store_name=${encodedStoreName}&close_date=${stockDate}`,
            {
              method: 'GET',
              headers: {
                'accept': 'application/json',
        ...
        // Parse responses (handle 404 as empty array)
        const openStockData = openStockResponse.ok ? await openStockResponse.json() : [];
        const closeStockData = closeStockResponse.ok ? await closeStockResponse.json() : [];

        // Combine the data into the expected format
        setStockTakeData({
          open_stocks: Array.isArray(openStockData) ? openStockData : [],
          close_stocks: Array.isArray(closeStockData) ? closeStockData : [],
  ...
  const stockTakeEntries: StockTakeEntry[] = useMemo(() => {
    ...
    const dateForDisplay = stockTakeData.stock_date || stockDate;
    ...
    productMap.forEach((data, key) => {
      ...
      entries.push({
        id: `${dateForDisplay}-${index}`,
        productName: productName,
        startDate: dateForDisplay,
        endDate: dateForDisplay,
