...
interface SummaryResponse {
  ...
  start_date: string;
  end_date: string;
  open_stock_entries: OpenStockEntry[];
  close_stock_entries: CloseStockEntry[];
  pos_sales: Record<string, number>;
...
export default function StockTakeLinesPage() {
    ...
    const fetchStockData = async () => {
      ...
      try {
        ...
        setStockTakeData({
          ...
          pos_sales: summaryData.pos_sales || {},
  ...
  const stockTakeEntries: StockTakeEntry[] = useMemo(() => {
    ...
    const openStocks: OpenStockEntry[] = stockTakeData.open_stocks || [];
    const closeStocks: CloseStockEntry[] = stockTakeData.close_stocks || [];
    const posSales: Record<string, number> = stockTakeData.pos_sales || {};
    const startDate = stockTakeData.start_date;
    const endDate = stockTakeData.end_date;

    // Create a map to combine open and close stock by product name and promoter
    ...
    openStocks.forEach((openStock) => {
      ...
      const existing = productMap.get(key) || { productName, openQty: 0, closeQty: 0 };
    ...
    closeStocks.forEach((closeStock) => {
    ...
    productMap.forEach((data) => {
      ...
      const diffQty = data.openQty - data.closeQty;
      // Get POS sales for this product from the pos_sales object
      const posTotalSale = posSales[productName] || 0;
      const variance = diffQty - posTotalSale;
